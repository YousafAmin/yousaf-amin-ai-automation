      ...
      <Container>
        <div className="flex items-center justify-between py-3.5">
          <a href="#home" className="group flex items-center gap-2.5" onClick={() => setOpen(false)}>
            <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-accent to-accent-3 text-sm font-bold text-white shadow-[0_0_20px_-4px_rgba(109,92,246,0.8)]">
              YA
            </span>
          ...
          <button
            ...
            aria-expanded={open}
            onClick={() => setOpen((v) => !v)}
            className="flex h-10 w-10 items-center justify-center rounded-full border border-border-strong bg-white/[0.03] text-foreground lg:hidden"
          >
